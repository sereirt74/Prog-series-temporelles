...
namespace RegArchLib {
	...
	cTarch::cTarch(const cTarch& theTarch) :cAbstCondVar(eTarch)
	{
		*this = theTarch;
		MESS_CREAT("cTarch")
	}
...
/*	cAbstCondVar* cTarch::PtrCopy() const
	{
		...
		return cAbstCondVarPtrCopy<cTarch>();
	}
*/
	...
	uint cTarch::GetNu(void) const
	{
		return mvArchPos.GetSize();
	}

	uint cTarch::GetNh(void) const
	{
		return 0;
	}
	...
	void cTarch::ComputeGradForM(uint theDate, const cRegArchValue& theValue, int theBegIndex, cFuncMeanAndVar& theDerivM, cAbstResiduals* theResids)
	{
		...
		{
			double myAux1 = theValue.mUt[theDate - i];
			if (myAux > 0)
			{
				theDerivM.mdFx[theBegIndex + i] = pow(myAux1, 2.0);
				theDerivM.mdFu[i - 1] = 2 * mvArchPos[i - 1] * myAux1;
			}
			else
			{
				theDerivM.mdFx[theBegIndex + i + myNArch] = pow(myAux1, 2.0);
				theDerivM.mdFu[i - 1] = 2 * mvArchNeg[i - 1] * myAux1;
			}
		}
	}
	...
	void cTarch::ComputeHessForM(uint theDate, const cRegArchValue& theValue, uint theBegIndex, cFuncMeanAndVar& theDerivM, cAbstResiduals* theResids)
	{
		uint myNArch = mvArchPos.GetSize();
		...
		for (uint i = 1; i <= MIN(theDate, myNArch); i++)
		{
			double myAux = theValue.mUt[theDate - i];
			if (myAux > 0)
				theDerivM.md2Fxu[i - 1][theBegIndex + i] = 2 * myAux;
			else
				theDerivM.md2Fxu[i - 1][theBegIndex + i + myNArch] = 2 * myAux;
		}
	}
	...
	void cTarch::GetParamName(uint theIndex, string theName[])
	{
	uint myIndex = theIndex;
	char myChar[100];
