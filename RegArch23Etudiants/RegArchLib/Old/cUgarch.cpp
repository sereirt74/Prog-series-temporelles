...
/*	cAbstCondVar* cUgarch::PtrCopy() const
	{
		...
		return cAbstCondVarPtrCopy<cUgarch>();
	}
*/
	...
	cDVector* myAux;
		...
		break;
		case 4:
			myAux = new cDVector(1, (double)mvExistConst);
			return *myAux;
		break;
		}
	}
	...
	*/
	...
	double myRes = 0;
		...
		for (uint i = 0; i < myNBeta ; i++)
			myRes += mvBetaS[i] * theData.mXvt[theDate][i] ;
	...
	uint cUgarch::GetNu(void) const
	{
		return mvArch.GetSize();
	}

	uint cUgarch::GetNh(void) const
	{
	...
	// Betas
		for (i = 1; i <= myNBeta; i++)
			theGradData.mCurrentGradVar[myBegIndex + i] = theValue.mXvt[theDate][i - 1];
	//ARCH
	...
	void cUgarch::ComputeGradForM(uint theDate, const cRegArchValue& theValue, int theBegIndex, cFuncMeanAndVar& theDerivM, cAbstResiduals* theResids)
	{
	}
	...
	void cUgarch::ComputeHessForM(uint theDate, const cRegArchValue& theValue, uint theBegIndex, cFuncMeanAndVar& theDerivM, cAbstResiduals* theResids)
	{
	...
	char myChar[100];
		...
		for (uint i = 0; i < mvBetaS.GetSize(); i++)
		{
			sprintf(myChar, "UGARCH BETA[%d]", i + 1);
			theName[myIndex++] = myChar;
