...
*/
	...
	*/
	...
	void cGarch::ComputeHess(uint theDate, const cRegArchValue& theData, const cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
	uint myp = mvArch.GetSize(), 
		myq = mvGarch.GetSize() ;
	...
	for (uint i = 1; i <= MIN(myp, theDate); i++)
		for (uint j = 0; j < myNParam; j++)
			myMat[myBegIndex + i][j] = -2.0 * theData.mUt[theDate - i] * theGradData.mGradMt[i - 1][j];
	theHessData.mCurrentHessVar += myMat;
	theHessData.mCurrentHessVar += Transpose(myMat);
	...
	for (uint j = 1; j <= MIN(myq, theDate); j++)
		for (uint k = 0; k < myNParam; k++)
			myMat[myBegIndex + myp + j][k] = theGradData.mGradHt[j - 1][k];
	theHessData.mCurrentHessVar += myMat;
	theHessData.mCurrentHessVar += Transpose(myMat);

	for (uint j = 0; j < MIN(myq, theDate); j++)
		theHessData.mCurrentHessVar += mvGarch[j] * theHessData.mHessHt[j];
	}
	
	void cGarch::ComputeGradAndHess(uint theDate, const cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
		uint myp = mvArch.GetSize(),
			myq = mvGarch.GetSize();
		uint myBegIndex = theGradData.GetNMeanParam();
		...
		//GARCH
		for (uint j = 1; j <= MIN(myq, theDate); j++)
			theGradData.mCurrentGradVar[myBegIndex + myp + j] += theData.mHt[theDate - j];
		for (uint j = 1; j <= MIN(myq, theDate); j++)
			theGradData.mCurrentGradVar += mvGarch[j - 1] * theGradData.mGradHt[j - 1];
