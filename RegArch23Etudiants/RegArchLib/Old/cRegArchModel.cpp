...
//		mVar = theCondVar.PtrCopy() ;
	...
	double mySigmat = sqrt(theValue.mHt[theDate]);
		...
		if (mMean != NULL)
			mMean->ComputeGrad(theDate, theValue, theGradData, mResids);
	...
	void cRegArchModel::ComputeGradWithM(uint theDate, cRegArchValue& theValue, cRegArchGradient& theGradData)
	{
		FillValue(theDate, *this, theValue);
		uint myBegIndex = mMean->GetNParam();
		mVar->ComputeGradWithM(theDate, theValue, theGradData, myBegIndex, mResids);
		double mySigmat = sqrt(theValue.mHt[theDate]);
		theGradData.mCurrentGradSigma = theGradData.mCurrentGradVar / (2.0 * mySigmat);
		if (mMean != NULL)
			mMean->ComputeGradWithM(theDate, theValue, theGradData, mResids);
		mResids->ComputeGrad(theDate, theValue, theGradData);
		theGradData.mCurrentGradEps = -1.0 * (theValue.mEpst[theDate] * theGradData.mCurrentGradSigma + theGradData.mCurrentGradMu) / mySigmat;
	}

	void cRegArchModel::ComputeGradWithF(uint theDate, cRegArchValue& theValue, cRegArchGradient& theGradData)
	{
		FillValue(theDate, *this, theValue);
		mVar->ComputeGrad(theDate, theValue, theGradData, mResids);
	double mySigmat = sqrt(theValue.mHt[theDate]);
		theGradData.mCurrentGradSigma = theGradData.mCurrentGradVar / (2.0 * mySigmat);
		if (mMean != NULL)
			mMean->ComputeGradWithF(theDate, theValue, theGradData, mResids);
		mResids->ComputeGrad(theDate, theValue, theGradData);
		theGradData.mCurrentGradEps = -1.0*(theValue.mEpst[theDate] * theGradData.mCurrentGradSigma + theGradData.mCurrentGradMu) / mySigmat;
	}
	...
	void cRegArchModel::ComputeGradAndHess(uint theDate, cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
		...
		if (mMean != NULL)
			mMean->ComputeHess(theDate, theData, theGradData, theHessData, mResids);
		mResids->ComputeHess(theDate, theData, theGradData, theHessData, mResids);
	...
	void cRegArchModel::ComputeGradAndHessWithF(uint theDate, cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
		ComputeGradWithF(theDate, theData, theGradData);
		mVar->ComputeHess(theDate, theData, theGradData, theHessData, mResids);
		if (mMean != NULL)
			mMean->ComputeHessWithF(theDate, theData, theGradData, theHessData, mResids);
		mResids->ComputeHess(theDate, theData, theGradData, theHessData, mResids);
		theHessData.ComputeHessSigmatFromHessVart(theDate, theData, theGradData);
		double mySigmat = sqrt(theData.mHt[theDate]);
		/* Hessien Eps(t, theta) */
	...
	void cRegArchModel::ComputeGradAndHessWithM(uint theDate, cRegArchValue& theValue, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
		FillValue(theDate, *this, theValue);
		uint myBegIndex = mMean->GetNParam();
		mVar->ComputeGradAndHessWithM(theDate, theValue, theGradData, theHessData, myBegIndex, mResids);
		double mySigmat = sqrt(theValue.mHt[theDate]);
		theGradData.mCurrentGradSigma = theGradData.mCurrentGradVar / (2.0 * mySigmat);
		if (mMean != NULL)
			mMean->ComputeGradAndHessWithM(theDate, theValue, theGradData, theHessData);
		mResids->ComputeHess(theDate, theValue, theGradData, theHessData, mResids);
		theGradData.mCurrentGradEps = -1.0 * (theValue.mEpst[theDate] * theGradData.mCurrentGradSigma + theGradData.mCurrentGradMu) / mySigmat;
...
/*
		ComputeGradWithM(theDate, theData, theGradData);
		uint myBegIndex = mMean->GetNParam();
		mVar->ComputeHessWithM(theDate, theData, theGradData, theHessData, myBegIndex, mResids);

		if (mMean != NULL)
			mMean->ComputeHessWithM(theDate, theData, theGradData, theHessData, mResids);
		mResids->ComputeHess(theDate, theData, theGradData, theHessData, mResids);
		theHessData.ComputeHessSigmatFromHessVart(theDate, theData, theGradData);
		double mySigmat = sqrt(theData.mHt[theDate]);
		theHessData.mCurrentHessEps = theGradData.mCurrentGradSigma * Transpose(theGradData.mCurrentGradMu) / theData.mHt[theDate];
