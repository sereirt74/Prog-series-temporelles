...
#else
*/
	double GslLLHFunction(const gsl_vector* theModel, void* theData)
	{
	pParamOptimPtr myParamOptim = (pParamOptimPtr)theData ;
	cRegArchModel* myParam = myParamOptim->mParam ;
	cRegArchValue* myValue = myParamOptim->mValue ;

	cDVector myTeta(theModel) ;

		myParam->VectorToRegArchParam(myTeta) ;
		return -1.0*RegArchLLH(*myParam, *myValue) ;
	}

	double NloptLLHFunction(uint theSize, double* theModel, void* theData)
	{
	pParamOptimPtr myParamOptim = (pParamOptimPtr)theData;
	cRegArchModel* myParam = myParamOptim->mParam;
	cRegArchValue* myValue = myParamOptim->mValue;
	cDVector myTeta(theSize, theModel);
...
#else
*/
	void GslGradLLHFunction(const gsl_vector* theModel, void* theData, gsl_vector* theGrad)
	{
	pParamOptimPtr myParamOptim = (pParamOptimPtr)theData ;
	cRegArchModel* myParam = myParamOptim->mParam ;
	cRegArchValue* myValue = myParamOptim->mValue ;
	...
	cDVector myGradLLH(theGrad) ;
		...
		myParam->VectorToRegArchParam(myTeta) ;
		RegArchGradLLH(*myParam, *myValue, myGradLLH) ;
		for (int i = 0 ; i < (int)theGrad->size ; i++)
			gsl_vector_set(theGrad, i, -myGradLLH[i]) ;
	}

	double NloptLLHAndGradLLHFunction(uint theSize, double* theModel, double* theGrad, void* theData)
	{
	pParamOptimPtr myParamOptim = (pParamOptimPtr)theData;
	cRegArchModel* myParam = myParamOptim->mParam;
	cRegArchValue* myValue = myParamOptim->mValue;
...
/*
	void NloptLLHAndGradLLHFunction(uint theSize, double* theModel, double& theLLH, double* theGrad, void* theData)
	{
		pParamOptimPtr myParamOptim = (pParamOptimPtr)theData;
		cRegArchModel* myParam = myParamOptim->mParam;
		cRegArchValue* myValue = myParamOptim->mValue;
		...
		if (myVerbose)
		{
			(myParamOptim->mNFuncEval)++;
#ifndef _RDLL_
			cout << "#Function evaluation: " << myParamOptim->mNFuncEval << " - Function value: " << theLLH << endl;
#else
			Rprintf("#Function evaluation: %d Function value: %f\n", myParamOptim->mNFuncEval, myLLH);
#endif //_RDLL_
		}
...
*/
	void GslLLHAndGradLLHFunction(const gsl_vector* theModel, void* theData, double* theLLH, gsl_vector* theGrad) 
	{
	pParamOptimPtr myParamOptim = (pParamOptimPtr)theData ;
	cRegArchModel* myParam = myParamOptim->mParam ;
	cRegArchValue* myValue = myParamOptim->mValue ;
