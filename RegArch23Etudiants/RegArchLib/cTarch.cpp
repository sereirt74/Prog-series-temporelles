...
*/
	...
	void cTarch::ReAlloc(const cDVector& theVectParam, const uint theNumParam)
	{
		...
		{
			case 1:
				mvArchPos = theVectParam;
			break;
			case 2:
				mvArchNeg = theVectParam;
			break;
			default:
			//			throw cError("cGarch::ReAlloc - theNumParam must be in 1, 2.") ;
			break;
	...
	void cTarch::ComputeHess(uint theDate, const cRegArchValue& theData, const cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
	uint myp = mvArchPos.GetSize();
	uint myBegIndex = theGradData.GetNMeanParam();
	...
	void cTarch::ComputeGradAndHess(uint theDate, const cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
		uint myp = mvArchPos.GetSize();
		theGradData.mCurrentGradVar = 0.0L;
		uint myBegIndex = theGradData.GetNMeanParam();
