...
*/
	...
	void cAr::ComputeHess(uint theDate, const cRegArchValue& theData, const cRegArchGradient& theGradData, cRegArchHessien& theHessData, uint theBegIndex)
	{
	// Hessien en AR = 0
	}

	void cAr::ComputeGradAndHess(uint theDate, const cRegArchValue& theValue, cRegArchGradient& theGradData, cRegArchHessien& theHessData, uint theBegIndex)
	{
		for (uint i = 1; i <= MIN(mvAr.GetSize(), theDate); i++)
			theGradData.mCurrentGradMu[theBegIndex + i - 1] += theValue.mYt[theDate - i];
		// Hessien en AR = 0
