...
//		void ReAllocMean(uint theNewSize) ; ///< Change the number of components for conditional mean
		...
		void ComputeGradWithF(uint theDate, cRegArchValue& theValue, cRegArchGradient& theGradData);
		void ComputeGradWithM(uint theDate, cRegArchValue& theValue, cRegArchGradient& theGradData);
		void NumericComputeGrad(uint theDate, cRegArchValue& theData, cRegArchGradient& theGradData, cAbstResiduals* theResiduals, cNumericDerivative& theNumDeriv);
		void RegArchParamToVector(cDVector& theDestVect) const ;
		void VectorToRegArchParam(const cDVector& theSrcParam) ;
		void ComputeGradAndHess(uint theDate, cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals);
		void ComputeGradAndHessWithF(uint theDate, cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals);
		void ComputeGradAndHessWithM(uint theDate, cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals);
		void NumericComputeGradAndHess(uint theDate, cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals, cNumericDerivative& theNumDeriv);
		void GetParamName(char** theName);
		void GetParamName(string theName[]);
		void GetParamName(cTabOfString& theName);
