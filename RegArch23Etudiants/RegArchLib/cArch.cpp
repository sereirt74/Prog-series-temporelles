...
//		 return mycArch;
		 return cAbstCondVarPtrCopy<cArch>();
	}
*/
#ifdef _RDLL_
	...
	*/
	...
	void cArch::ComputeHess(uint theDate, const cRegArchValue& theData, const cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
	uint myp = mvArch.GetSize();
	uint myBegIndex = theGradData.GetNMeanParam();
	...
	void cArch::ComputeGradAndHess(uint theDate, const cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
	uint myp = mvArch.GetSize();
	uint myBegIndex = theGradData.GetNMeanParam();
	uint myNParam = theGradData.GetNParam();
	...
	void cArch::GetParamName(uint theIndex, string theName[])
	{
	uint myIndex = theIndex;
	char myChar[100];
