...
namespace RegArchLib {
	 ...
	 */
/*
cAbstCondMean* cAr::PtrCopy(void)
	{
		return cAbstCondMeanPtrCopy<cAr>();
	}
*/
	/*!
...
//		return mvAr.GetSize();
	...
	uint cAr::GetNu(void) const
	{
		return 0;
	}
	uint cAr::GetNh(void) const
	{
		return 0;
	}
	...
	{
		for (uint i = 1 ; i <= MIN(mvAr.GetSize(), theDate) ; i++)
			theGradData.mCurrentGradMu[theBegIndex+i-1] += theValue.mYt[theDate - i] ;
	}

	void  cAr::GetNParamF(uint theNParam[3]) const
	{
		theNParam[0] = mvAr.GetSize();
		theNParam[1] = theNParam[2] = 0; 
	}

	void cAr::ComputeGradF(uint theDate, const cRegArchValue& theValue, cDVector& theGradF)
	{
	uint myp = mvAr.GetSize();
		for (uint i = 1; i <= MIN(myp, theDate); i++)
			theGradF[i - 1] = theValue.mYt[theDate - i];
	}

	void cAr::ComputeGradForM(uint theDate, const cRegArchValue& theValue, int theBegIndex, cDVector& theGradTheta, cDVector& theGradU, double& theGradH, uint& theNu, uint& theNh)
	{
		theNu = theNh = 0;
		theGradTheta = 0.0;
		uint myNAr = mvAr.GetSize();
		for (uint i = 1 ; i <= MIN(myNAr, theDate) ; i++)
			theGradTheta[theBegIndex+i-1] = theValue.mYt[theDate-i];
	}

	void cAr::ComputeGradForM(uint theDate, const cRegArchValue& theValue, int theBegIndex, cFuncMeanAndVar& theDerivM)
	{
		theDerivM.mdFx = 0.0;
		uint myNAr = mvAr.GetSize();
		for (uint i = 1; i <= MIN(myNAr, theDate); i++)
			theDerivM.mdFx[theBegIndex + i - 1] = theValue.mYt[theDate - i];
	}
	...
	void cAr::ComputeHessForM(uint theDate, const cRegArchValue& theValue, uint theBegIndex, const cDVector& theGradTheta, const cDVector& theGradU, double theGradH, uint theNu, uint theNh, cDMatrix& theHessTheta2,cDVector* theHessThetaU, cDVector& theHessThetaH, cDVector* theHessU2, cDVector& theHessUH, double& theHessH2)
	{
//		ComputeGradForM(theDate, theValue, theBegIndex, theGradTheta, theGradU, theGradH, theNu, theNh);
	}

	void cAr::ComputeHessForM(uint theDate, const cRegArchValue& theValue, uint theBegIndex, cFuncMeanAndVar& theDerivM)
	{

	}

	void cAr::ComputeHessF(uint theDate, const cRegArchValue& theData, cDMatrix& theHessF)
	{
		theHessF = 0.0;
	}
	...
	void cAr::GetParamName(uint theIndex, string theName[])
	{
	uint myIndex = theIndex;
	char myChar[100];
		for (uint i = 0; i < mvAr.GetSize(); i++)
