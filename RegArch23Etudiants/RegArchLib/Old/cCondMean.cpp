...
namespace RegArchLib {
	...
	*/
	...
	void cCondMean::ComputeGradWithF(uint theDate, const cRegArchValue& theValue, cRegArchGradient& theGradData, cAbstResiduals* theResids)
	{
	uint myIndex = 0;
		theGradData.mCurrentGradMu = 0.0L;
		for (uint i = 0; i < MAXCONDMEAN; i++)
			if (mvCondMean[i] != NULL)
			{
				mvCondMean[i]->ComputeGradWithF(theDate, theValue, theGradData, myIndex, theResids);
				myIndex += mvCondMean[i]->GetNParam();
			}
	}

	void cCondMean::ComputeGradWithM(uint theDate, const cRegArchValue& theValue, cRegArchGradient& theGradData, cAbstResiduals* theResids)
	{
		uint myIndex = 0;
		theGradData.mCurrentGradMu = 0.0L;
		for (uint i = 0; i < MAXCONDMEAN; i++)
			if (mvCondMean[i] != NULL)
			{
				mvCondMean[i]->ComputeGradWithM(theDate, theValue, theGradData, myIndex, theResids);
				myIndex += mvCondMean[i]->GetNParam();
			}
	}
...
*/
	...
	void cCondMean::ComputeHessWithF(uint theDate, const cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
	uint myIndex = 0;
		theHessData.mCurrentHessMu = 0.0;
		for (uint i = 0; i < MAXCONDMEAN; i++)
			if (mvCondMean[i] != NULL)
			{
				mvCondMean[i]->ComputeHessWithF(theDate, theData, theGradData, theHessData, myIndex, theResiduals);
				myIndex += mvCondMean[i]->GetNParam();
			}
	}

	void cCondMean::ComputeHessWithM(uint theDate, const cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
		uint myIndex = 0;
		theHessData.mCurrentHessMu = 0.0;
		for (uint i = 0; i < MAXCONDMEAN; i++)
			if (mvCondMean[i] != NULL)
			{
				mvCondMean[i]->ComputeHessWithM(theDate, theData, theGradData, theHessData, myIndex, theResiduals);
				myIndex += mvCondMean[i]->GetNParam();
			}
	}

	void cCondMean::ComputeGradAndHessWithF(uint theDate, const cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
	{
		uint myIndex = 0;
		theHessData.mCurrentHessMu = 0.0;
		for (uint i = 0; i < MAXCONDMEAN; i++)
			if (mvCondMean[i] != NULL)
			{
				mvCondMean[i]->ComputeGradAndHessWithF(theDate, theData, theGradData, theHessData, myIndex, theResiduals);
				myIndex += mvCondMean[i]->GetNParam();
			}
	}

	void cCondMean::ComputeGradAndHessWithM(uint theDate, const cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData)
	{
		uint myIndex = 0;
		theHessData.mCurrentHessMu = 0.0;
		for (uint i = 0; i < MAXCONDMEAN; i++)
			if (mvCondMean[i] != NULL)
			{
				mvCondMean[i]->ComputeGradAndHessWithM(theDate, myIndex, theData, theGradData, theHessData);
			}
	}
