...
namespace RegArchLib {
	...
	*/
	...
	T*	mySrc = static_cast<T *>(theAbstCondVar);
		if (mySrc)
		{
			//		static std::shared_ptr<T> myCondVar(new T(*mySrc));
			//			return &*myCondVar;
			return new T(*mySrc);
		}
		else
	...
	*/
	...
	{
		//	static std::shared_ptr<T> myCondVar(new T());
		//		return &*myCondVar;
		return new T();
	}
	...
	*/
	...
	void cAbstCondVar::ComputeGradWithM(uint theDate, const cRegArchValue& theValue, cRegArchGradient& theGradData, uint theBegIndex, cAbstResiduals* theResiduals)
	{
		uint myNu = GetNu();
		uint myNh = GetNh();
		uint myNtheta = theGradData.mCurrentGradMu.GetSize();
		cFuncMeanAndVar myDerivM = cFuncMeanAndVar(myNtheta, false, myNu, myNh);
		ComputeGradForM(theDate, theValue, theBegIndex, myDerivM);
		myDerivM.ComputeGradFunc(theGradData);
	}

	void cAbstCondVar::ComputeHessWithM(uint theDate, const cRegArchValue& theData, const cRegArchGradient& theGradData, cRegArchHessien& theHessData, uint theBegIndex, cAbstResiduals* theResiduals)
	{
		uint myNu = GetNu();
		uint myNh = GetNh();
		uint myNtheta = theGradData.mCurrentGradMu.GetSize();
	...
	void cAbstCondVar::ComputeGradAndHessWithM(uint theDate, const cRegArchValue& theValue, cRegArchGradient& theGradData, cRegArchHessien& theHessData, uint theBegIndex, cAbstResiduals* theResiduals)
	{
		uint myNu = GetNu();
		uint myNh = GetNh();
		uint myNtheta = theGradData.mCurrentGradMu.GetSize();
